...
httpx ASGITransport — real HTTP semantics, no sockets, no orchestrator. This
is the offline stand-in for the go-livepeer trickle broker (plan §3.10) and
proves the documented channel contract end to end: frames on video-in reach the
session step and observations land on events-out; control messages round-trip.
...
def _rail(rail_client, session_id="s1", token="t", control_url=BASE):
    return TrickleRail(
        control_url=control_url, session_id=session_id, token=token, client=rail_client
    )
...
def test_open_maps_channel_endpoints():
    async def main():
...
def test_frame_flows_video_in_to_events_out():
    async def main():
        ...
        await hub_client.post(
            f"{ep.video_in}/1",
            content=JPEG,
            headers={"X-Livepeer-Timestamp": "12.500"},
        )
        ...
        assert int(ev.headers["X-Livepeer-Seq"]) == 1
        ...
        assert int(ev2.headers["X-Livepeer-Seq"]) == 2
